...
export default async function Home() {
  ...
  const [latestDropsData, bestsellersData, siteConfigData] = await Promise.all([
    getProducts({ sortKey: 'CREATED_AT', reverse: true, limit: 4 }),
    getCollectionProducts({ collection: 'bestsellers', limit: 4 }),
  ...
  return (
    <>
      ...
      <section className="py-8 md:py-12 lg:py-16 bg-muted/5">
        <div className="w-full px-4 md:px-8">
          ...
          {bestsellers.length > 0 && (
            <>
              <BestsellersGrid products={bestsellers.map(mapProduct)} />
      ...
      {/* Latest Drops Section - Show demo if no products */}
